...
#  Loop through files 
#######################################
tt=1															# Calculate length of forcing data files (number of rows)
...
# 	os.mkdir('/Users/cjh458/Desktop/CREATE_snoTEL_ATTRIBUTES')	# Create directory that will contain the attribute files
	...
	dimid_hru = ncid.createDimension('hru',76088)					# Declare hydrological response unit dimension of 1
	dimid_gru = ncid.createDimension('gru',76088)					# Declare geographical response unit dimension of 1
	...
	# Assign variable  value
	hruId_varid[:]= 103473 #h2g_data[hruid[z]]#hruid[z]						# Assign variable a value from the snoTEL information .csv
	...
	# Assign variable  value
	hru2gruId_varid[:] = 103473 #h2g_data[hruid[z]] #hruid[z]				# assign new variable existing value for the HRU
	...
	h2g_data = np.copy(raw_data)									# Copy this variable to a numpy array
	gruId_varid[:] = 103473 #h2g_data[hruid[z]]		#hruid[z				# assign new variable existing value for the HRU
	...
	soilti_varid[:] = soilti_data[hruid[z]]							# assign new variable existing value for the HRU
