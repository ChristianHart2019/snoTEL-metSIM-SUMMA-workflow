...
outfile ='snotel_forcing.nc'								# Create name for an output file from and the existing input file
...
for z in range(tt):											# Iterate through snotel sites
	wind_varid[:,z]=0#wind_data								# Assign NLDAS data to file
...
for z in range(tt):											# Iterate through snotel sites
	...
	time_varid[:,z] = timedata								# Update the original time stamp to the converted one
...
for z in range(tt):											# Iterate through snotel sites
	...
	LWRadAtm_varid[:,z] = data								# Write date to LWRadAtm
...
for z in range(tt):											# Iterate through snotel sites
	...
	data = np.copy(raw_data)								# Make a copies of these variables
	SWRadAtm_varid[:,z] = data								# Write data to SWRadAtm	
...
for z in range(tt):											# Iterate through snotel sites
	...
	pres_varid[:,z] = presdata								# Update the original air pressure to the converted one
...
for z in range(tt):											# Iterate through snotel sites
	...
	temp_varid[:,z] = tempdata								# Update the original air temperature to the converted one
...
for z in range(tt):											# Iterate through snotel sites
	...
	prec_varid[:,z] = prec_data								# Update the original precipitation data and add to the converted dataset 
...
for z in range(tt):											# Iterate through snotel sites
	...
	spechum_varid[:,z] = spechum_data						# Update the original specific humidity data and add to the converted dataset 
